...
class ViewController: ThemeAwareViewController {
  // MARK: - Properties
...
extension ViewController: UISearchControllerDelegate, UISearchResultsUpdating, UISearchBarDelegate {
  ...
  func searchBarShouldBeginEditing(_ searchBar: UISearchBar) -> Bool {
    navigationController?.pushViewController(SearchResultsViewController(viewModel: dataViewModel), animated: true)
    return true
  }
  ...
  func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
    ...
    // 推送到搜尋結果頁面
    navigationController?.pushViewController(SearchResultsViewController(viewModel: dataViewModel), animated: false)
  }
}
