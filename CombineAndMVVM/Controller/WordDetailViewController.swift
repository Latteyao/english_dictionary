...
/// 單字詳細資訊頁面
class WordDetailViewController: ThemeAwareViewController {
  // MARK: - Properties
...
//  private var cancellables: AnyCancellable?
  ...
  override func viewDidLoad() {
    super.viewDidLoad()
    view.backgroundColor = ThemeManager.currentBackgroundColor // 注意 需要設定背景顏色 不然切換動畫會卡
...
extension WordDetailViewController {
  ...
  @objc func speakButtonPressed() {
    ...
    let speechUtterance = AVSpeechUtterance(string: dataViewModel.viewData.word)
  ...
  @objc func bookmarkButtonPressed() {
    guard let dataViewModel = dataViewModel else { return }
    let word = dataViewModel.viewData.word
    if isBookmarked {
